...
def loja(request, filtro=None):
    ...
    if request.method == 'POST':
        ...
        # aplica filtro só se tiver valor
        if preco_minimo:
            produtos = produtos.filter(preco__gte=float(preco_minimo))

        if preco_maximo:
            produtos = produtos.filter(preco__lte=float(preco_maximo))
...
def finalizar_pedido(request, id_pedido):
    if request.method == 'POST':
        ...
        if not 'endereco' in dados:
            ...
            id_endereco = int(dados.get('endereco'))
...
def adicionar_endereco(request):
    if request.method == 'POST':
        ...
        endereco = Endereco.objects.create(cliente=cliente, rua=dados.get('rua'), numero = int(dados.get('numero')), 
                                           estado = dados.get('estado'), cidade = dados.get('cidade'), cep= dados.get('cep'), complemento = dados.get('complemento'))
        endereco.save()
        return redirect('checkout')
    else:
        context = {}
...
def minha_conta(request):
    ...
    if request.method == 'POST':
        ...
        if 'senha_atual' in dados:
            ...
            #esta modificando os dados pessoais
            email = dados.get('email')
        ...
        if 'email' in dados and 'senha' in dados and 'confirmacao_senha' in dados:
            ...
            if senha == confirmacao_senha:
                ...
                if not criado:
                    ...
                    # criar cliente
                    # verificar se existe id_sessao nos cookies do navegador
                    if request.COOKIES.get('id_sessao'):
                        id_sessao = request.COOKIES.get('id_sessao')
                        cliente, criado = Cliente.objects.get_or_create(id_sessao=id_sessao)
                    else:
